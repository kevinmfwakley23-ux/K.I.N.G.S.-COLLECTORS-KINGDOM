...
test("macro grading UI analyzes dedicated corner and edge captures and persists only SHA-linked advisory evidence", async () => {
  ...
  assert.match(source, /region:\s*currentResult\.region/);
  ...
  assert.match(source, /do not confirm whitening, trimming, authenticity, condition, value, or an official grade/i);
