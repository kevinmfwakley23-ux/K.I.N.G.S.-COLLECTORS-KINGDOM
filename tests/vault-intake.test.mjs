...
test("Royal Intake Queue validates identifier types and structured barcode/card/cert patterns", async () => {
  await withIntake(async ({ intakeService }) => {
    ...
    assert.throws(() => intakeService.capture(collectorA, {
      ...
      identifierValue: "123"
    }), /Identifier type must be barcode, UPC, EAN, ISBN, Pokémon card ID, Pokémon set\/card number, Magic Scryfall ID, Magic set\/collector number, PSA certification number, sports-card UCID, sports-card set\/card number, catalog, serial, SKU, or custom/i);
