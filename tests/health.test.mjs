...
test("health snapshot reports deterministic service state", () => {
  ...
  assert.equal(snapshot.service, "kings-collectors-kingdom");
  assert.equal(snapshot.status, "ok");
  assert.equal(snapshot.version, "1.2.3");
  assert.equal(snapshot.uptimeMs, 5000);
});

test("readiness fails closed until configuration is loaded", () => {
  assert.equal(createReadinessSnapshot().status, "not-ready");
  assert.equal(createReadinessSnapshot({ configLoaded: true }).status, "ready");
});
