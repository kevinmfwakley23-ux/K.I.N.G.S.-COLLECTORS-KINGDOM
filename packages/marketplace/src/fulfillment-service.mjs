...
function publicShipment(shipment) {
  return Object.freeze({
    ...
    verificationState: "seller-declared",
    evidenceSha256: shipment.requestSha256,
    trackingProvided: Boolean(shipment.trackingNumber),
    declaredShippedAt: shipment.declaredShippedAt,
    createdAt: shipment.createdAt,
    carrierVerified: false,
    carrierScanAuthorityAvailable: false,
    deliveryVerified: false,
    signatureConfirmationVerified: false,
...
function publicShipmentEvent(entry) {
  return Object.freeze({
    id: entry.id,
...
function fulfillmentSummary(order, shipments) {
  ...
  return Object.freeze({
    ...
    evidenceAuthority: shippedQuantity ? "seller-declared" : null,
    carrierVerificationAvailable: false,
    deliveryVerificationAvailable: false,
    deliveryVerified: false,
    signatureConfirmationVerificationAvailable: false,
    buyerProtectionDecisionAvailable: false,
    ownershipTransferAuthorized: false,
    soldProvenanceEventCreated: false
  });
...
function publicOrder(order, shipments, evidenceEvents = undefined) {
  return Object.freeze({
    id: order.id,
    ...
    ...(evidenceEvents ? { fulfillmentEvidenceEvents: Object.freeze(evidenceEvents.map(publicShipmentEvent)) } : {}),
    ownershipTransferAuthorized: false,
    soldProvenanceEventCreated: false
  });
}
...
export function createMarketplaceFulfillmentService({ fulfillmentRepository, now = () => new Date() } = {}) {
  ...
  function getOrderFulfillment(identity, orderIdValue) {
    ...
    return publicOrder(
      order,
      fulfillmentRepository.listOrderShipments(order.id),
      fulfillmentRepository.listOrderShipmentEvents(order.id)
    );
  }
