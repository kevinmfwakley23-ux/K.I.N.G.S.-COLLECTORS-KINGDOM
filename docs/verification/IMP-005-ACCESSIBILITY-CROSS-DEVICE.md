...
Status: **MANUAL ACCEPTANCE OPEN — automated safeguards verified; real device/assistive-technology pass still required before IMP-005 completion**

This record separates what the repository can prove automatically from what must be checked with a real browser, physical/representative viewport behavior, and assistive technology. Passing source-level tests is not treated as screen-reader certification or physical-device acceptance.

The authoritative overall phase matrix is [`IMP-005-ACCEPTANCE.md`](IMP-005-ACCEPTANCE.md).
...
- meaningful `Photo of <treasure title>` alternatives for collector images and hidden semantics for decorative imagery;
...
- phone-width collapse of Vault workspace, search/form grids, sidebar, collection grid, details, managers, and dialogs;
- responsive top-bar behavior at 820px and 640px;
- production packaging of accessibility JS/CSS so a release fails if those assets disappear.

Grounded Royal Curator tag suggestions and possible-duplicate descriptions are also required to remain informational/advisory rather than silently representing actions already taken.
...
2. Reach every top-bar, search/filter, system-view, folder/location, treasure, pagination, Keeper, import, set, tag-recommendation, Marketplace-preparation, evidence, provenance, and destructive-action control using keyboard only.
...
- tag recommendations are announced as suggestions from the collector's own Vault and not as automatic edits;
- duplicate candidates are announced as possible matches and not as merged/deleted records;
...
- long collectible names, locations, categories, provenance text, recommendation explanations, and checklist names remain discoverable.
...
| --- | --- | --- |
| Small Android phone | ~360×800 CSS px | navigation, search, add/edit, detail dialog, image upload, recommendations, sets, Marketplace preparation, Keeper |
...
Do not mark this complete from CI, source inspection, screenshots, or emulated viewport width alone.

Until those conditions are met, PR #7 must remain **DRAFT** and accessibility/cross-device verification must remain explicitly open.
